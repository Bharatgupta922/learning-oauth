...
const DashboardPage = async () => {
  ...
  return (
    ...
    <div className='pb-20 text-primary-foreground text-2xl'>You are logged in AS <span className='bold text-6xl'>{user?.name}</span> with EmailId <span className='bold text-6xl'>{user.email}</span> </div>
        ...
        <div className=" gap-2 border p-4 rounded-lg bg-gray-100 transition-all cursor-pointer hover:shadow-xl">
        {user?.picture && (<Image alt = "PROFILE_PICTURE" src = {user.picture} className='rounded-full size-16' height={40} width={60}/>)}
    ...
    {/* <div className="absolute right-4top-4 ">
